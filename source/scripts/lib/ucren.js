...
let blankArray = [];
let slice = blankArray.slice;
let join = blankArray.join;
...
Function.prototype.saturate = function(scope/* , args */) {
	let fn = this; let afters = slice.call( arguments, 1 );
	return function() {
...
Array.prototype.random = function() {
	let arr = this.slice( 0 ); let ret = []; let i = arr.length;
	while ( i -- ) {
...
Ucren = {
	...
	Class: function(initialize, methods, befores, afters) {
		let fn; let prototype; let blank;
	...
	// Ucren.randomWord
	randomWord: function() {
		let cw = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
...
Ucren.BasicDrag = Ucren.Class(
	/* constructor */ function(conf) {
		...
		let isTouch = this.isTouch = 'ontouchstart' in window;
	...
	/* methods */ {
		...
		getCoors: function(e) {
			...
			if (e.targetTouches && e.targetTouches.length) { 	// iPhone
				let thisTouch = e.targetTouches[0];
		...
		unRegisterDocumentEvent: function() {
			let draging = this.target.draging;
...
Ucren.BasicElement = Ucren.Class(
	...
	/* methods */ {
		...
		getPos: function() {
			let el; let parentNode; let pos; let box; let offset;
			el = this.dom;
			pos = {};
			...
			if (el.getBoundingClientRect) {
				...
				let doc = document;
				let scrollTop = Math.max(doc.documentElement.scrollTop,
					doc.body.scrollTop);
				let scrollLeft = Math.max(doc.documentElement.scrollLeft,
					doc.body.scrollLeft);
				return {
		...
		setAlpha: function() {
			let reOpacity = /alpha\s*\(\s*opacity\s*=\s*([^\)]+)\)/;
			return function(value) {
				let element = this.dom; let es = element.style;
				if (!Ucren.isIe) {

...
ClassTimer.prototype.getFPS = function() {
	let t = now(); let c = this.count; let fps = c / ( t - this.startTime ) * 1e3;
	if ( c > 1e3 ) {
		this.count = 0,
		this.startTime = t;
	}
...
ClassTimer.prototype.update = function( time ) {
	let tasks = this.tasks; let addingTasks = this.addingTasks; let adding = this.adding;
	let i = tasks.length; let t; let task; let start; let duration; let data;
